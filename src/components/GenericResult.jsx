...
export default function GenericResult({ goTo, state }) {
  ...
  useEffect(() => {
    if (result.meterPct !== undefined) {
      const t = setTimeout(() => {
        if (meterRef.current) meterRef.current.style.width = result.meterPct + '%'
      }, 200)
  ...
  return (
    <div className="glass-card rounded-[24px] px-7 py-10 sm:px-11 sm:py-14 text-center" style={{ paddingBottom: 80 }}>
      ...
      {result.meterPct !== undefined && (
        <div className="mb-7">
          <div className="flex justify-between text-xs font-semibold mb-2.5">
            <span style={{ color: 'var(--quiz-color)' }}>{result.meterLeft}</span>
            <span className="text-[#7e79a0]">{result.meterRight}</span>
          </div>
          <div className="h-2.5 bg-white/6 rounded-full overflow-hidden">
            <div ref={meterRef} className="meter-fill" style={{ width: '0%', background: 'var(--quiz-color)' }} />
          </div>
        </div>
      )}
